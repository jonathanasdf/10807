...
local function shortcut(nInputPlane, nOutputPlane, stride)
  ...
  if stride == 1 and nInputPlane == nOutputPlane then
    ...
    return nn.SpatialConvolution(nInputPlane, nOutputPlane, 1, 1, stride, stride, 0, 0):noBias()
  end
end
...
function BasicResidualModule:__tostring__()
  return string.format('%s(%d, %d, %d, %f)', torch.type(self), self.nInputPlane, self.n, self.stride, self.dropout)
end
...
function BottleneckResidualModule:__tostring__()
  return string.format('%s(%d, %d, %d, %d, %f)', torch.type(self), self.nInputPlane, self.nSqueeze, self.nExpand, self.stride, self.dropout)
end
